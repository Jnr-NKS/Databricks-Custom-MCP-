...
def _format_column_details_md(columns: List[ColumnInfo]) -> List[str]:
    ...
    for col in columns:
        if not isinstance(col, ColumnInfo):
            print(f"Warning: Encountered an unexpected item in columns list: {type(col)}. Skipping.", file=sys.stderr)
            continue
...
def _process_lineage_results(lineage_query_output: Dict[str, Any], main_table_full_name: str) -> Dict[str, Any]:
    ...
    if not lineage_query_output or lineage_query_output.get("status") != "success" or not isinstance(lineage_query_output.get("data"), list):
        print("Warning: Lineage query output is invalid or not successful. Returning empty lineage.", file=sys.stderr)
...
def _get_table_lineage(table_full_name: str) -> Dict[str, Any]:
    ...
    """
    print(f"Fetching and processing lineage for table: {table_full_name}", file=sys.stderr)
...
def get_uc_schema_details(catalog_name: str, schema_name: str, include_columns: bool = False) -> str:
    """
    Fetches detailed information for a specific schema within a Unity Catalog.
        ...
        if not tables_list:
            ...
            for i, table_info in enumerate(tables_list):
                if not isinstance(table_info, TableInfo):
                    print(f"Warning: Encountered an unexpected item in tables list: {type(table_info)}", file=sys.stderr)
                    continue
...
def get_uc_catalog_details(catalog_name: str) -> str:
    ...
    try:
        print(f"Fetching schemas for catalog: {catalog_name} using global sdk_client...", file=sys.stderr)
        ...
        for i, schema_info in enumerate(schemas_list):
            if not isinstance(schema_info, SchemaInfo):
                print(f"Warning: Encountered an unexpected item in schemas list: {type(schema_info)}", file=sys.stderr)
                continue
    ...
    try:
        ...
        for catalog_info in catalogs_list:
            if not isinstance(catalog_info, CatalogInfo):
                print(f"Warning: Encountered an unexpected item in catalogs list: {type(catalog_info)}", file=sys.stderr)
                continue
